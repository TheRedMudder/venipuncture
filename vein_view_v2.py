"""
Vein Viewer v2 — Basler a2A3536-9gmBAS
Live NIR vein capture with on-the-fly tuning.

Panels: RAW | ENHANCED (CLAHE)  [+ VESSELS (Frangi) when toggled]

Keys:
  Up / Down     exposure +/- 1000 us
  Left / Right  CLAHE clipLimit -/+ 0.5
  F             toggle Frangi vessel filter panel (slower)
  S             save frames to captures/YYYY-MM-DD/ (settings in filename)
  Q             quit

Setup (once):
    pip install pypylon opencv-python scikit-image numpy

Run (close pylon Viewer first — only one program can own the camera):
    python vein_view_v2.py
"""
...
def vessels(img):
    """Frangi vesselness: responds to dark tube-like structures (veins).
    ...
    # black_ridges=True because veins are darker than surrounding skin
...
while camera.IsGrabbing():
    ...
    cv2.imshow("Vein Viewer v2", cv2.vconcat([combo, bar]))
